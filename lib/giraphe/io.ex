...
defmodule Giraphe.IO do
  ...
  defp get_query_output(query_result, default_fun) do
    case query_result do
      ...
      {:error, target, object, reason} ->
        :ok =
          Logger.warn(
            "Unable to query target '#{target}' for object '#{object}': #{inspect(reason)}"
          )
      ...
      routes =
        if length(routes0) <= length(addresses) do
          Enum.map(addresses, fn address ->
            {NetAddr.first_address(address), address_to_self_address(address)}
          end)
        else
          routes0
        end
  ...
  def fetch_switch(target, physaddr, gateway_mac) do
    switch = %Switch{polladdr: target, physaddr: physaddr}

    if is_snmp_agent(target) do
      switch = %{switch | name: get_target_sysname(target), fdb: get_target_fdb(target)}

      %{switch | uplink: get_switchport_by_mac(switch, gateway_mac)}
    else
      %{switch | name: NetAddr.address(target), fdb: []}
    end
  end
  ...
  def export_routers(routers, routers_file) do
    ...
    with {:error, error} <-
           File.write(routers_file, "[#{string}]") do
  ...
  def export_hosts(hosts, hosts_file) do
    ...
    with {:error, error} <- File.write(hosts_file, string) do
  ...
  def render_l2_graph(
    ...
    with {:error, error} <- File.write(output_file, graph) do
